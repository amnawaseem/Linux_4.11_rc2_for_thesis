...
static void vcpu_self_mark_events_pending(struct shared_info *s, uint16_t notify_cpu_id )
{
    unsigned int val = 1;
    struct vcpu_info *vcpu_info = &(s->vcpu_info[notify_cpu_id]);
    int already_pending = sync_test_and_set_bit(
    ...
    if ( already_pending )
        return;
   // Capability 1 is for guest to use target itselt
   // For capability 1 ( second cap) in scenario, set target to itself
    //vgic_vcpu_inject_irq(v, v->domain->arch.evtchn_irq);
   asm volatile("mov x0, #0x9999\n\tmov x1, %0\n\thvc #0" :: "r" (val) : "x0", "x1");
}
...
int evtchn_send(unsigned int lport)
{
    ...
    case ECS_INTERDOMAIN:
        ...
        if ( !sync_test_bit(rport, BM(&s->evtchn_mask[0])) &&
             !sync_test_and_set_bit(rport / BITS_PER_EVTCHN_WORD,
					   BM(&s->vcpu_info[0].evtchn_pending_sel)) )
        {
            if (rd != CONFIG_XEN_DOM_ID)
                vcpu_mark_events_pending(s,rchn->notify_vcpu_id);
            else
                vcpu_self_mark_events_pending(s,rchn->notify_vcpu_id);
        }
...
int bind_interdomain_evtchn_to_irq(struct evtchn_bind_interdomain *interdomain)
{
    ...
    bind_interdomain.remote_dom  = interdomain->remote_dom;
    bind_interdomain.remote_port = interdomain->remote_port;
    remote_event_channel = (struct evtchn *)(evtchn_domains + (bind_interdomain.remote_dom * EVTCHNS_PER_BUCKET));
    if (fifo_events)
    ...
    else
    {
        ...
        if ( !port_is_valid(bind_interdomain.remote_port) )
            return -EINVAL;
        ...
        interdomain->local_port = bind_interdomain.local_port;
...
int bind_interdomain_evtchn(struct evtchn_bind_interdomain *interdomain)
{
    struct evtchn_bind_interdomain bind_interdomain;
    int err = -1;
    struct evtchn *lchn, *rchn;
   // struct shared_info *s;
    ...
    else
    {
        ...
        bind_interdomain.local_port = local_port;
        interdomain->local_port = bind_interdomain.local_port;
       /* s = (struct shared_info *)(Shared_info_pages + (XEN_PAGE_SIZE * bind_interdomain.remote_dom));
	    if (!(sync_test_and_set_bit(local_port, BM(&s->evtchn_pending[0])))){
            if ( !sync_test_bit(local_port, BM(&s->evtchn_mask[0])) &&
                 !sync_test_and_set_bit(local_port / BITS_PER_EVTCHN_WORD,
    					   BM(&s->vcpu_info[0].evtchn_pending_sel)) )
            {
                if (bind_interdomain.remote_dom != CONFIG_XEN_DOM_ID)
                    vcpu_mark_events_pending(s,rchn->notify_vcpu_id);
                else
                    vcpu_self_mark_events_pending(s,rchn->notify_vcpu_id);
            }
...
EXPORT_SYMBOL_GPL(bind_interdomain_evtchn);
					  ...
					  void *dev_id)
{
    ...
    interdomain.remote_dom = remote_domain;
    interdomain.remote_port = remote_port;
	irq = bind_interdomain_evtchn_to_irq(&interdomain);
	if (irq < 0)
		return irq;
